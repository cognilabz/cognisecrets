...
func TestReconcileCreatesComposedTarget(t *testing.T) {
	...
	if target.Labels[cogniv1.ManagedByLabel] != cogniv1.ManagedByValue {
...
func TestReconcileDeletesManagedTargetOnAccessDenied(t *testing.T) {
	...
	assertReady(t, reconciler.Client, ctx, ref, metav1.ConditionFalse, cogniv1.ReasonAccessDenied)
}
...
func TestReconcileReportsSourceNotFound(t *testing.T) {
	...
	assertReady(t, reconciler.Client, ctx, ref, metav1.ConditionFalse, cogniv1.ReasonSourceNotFound)
}

func TestReconcileDeletesManagedTargetOnMissingKey(t *testing.T) {
	ctx := context.Background()
	ref := newReference("application", "database", []cogniv1.SecretSource{{
		Namespace: "shared",
		Name:      "database",
		Keys:      []cogniv1.SecretKeyMapping{{Name: "password"}},
	}})
	...
	assertReady(t, reconciler.Client, ctx, ref, metav1.ConditionFalse, cogniv1.ReasonSourceKeyNotFound)
}
...
func TestReconcileRejectsDuplicateTargetKeyWithContributors(t *testing.T) {
	...
	ref := newReference("application", "database", []cogniv1.SecretSource{
		{
			Namespace: "shared",
			Name:      "database",
			Keys:      []cogniv1.SecretKeyMapping{{Name: "password", Target: "PASSWORD"}},
		},
		{
			Namespace: "shared",
			Name:      "messaging",
			Keys:      []cogniv1.SecretKeyMapping{{Name: "token", Target: "PASSWORD"}},
		},
	})
	...
	if condition.Reason != cogniv1.ReasonDuplicateTargetKey {
		t.Fatalf("reason = %q, want %q", condition.Reason, cogniv1.ReasonDuplicateTargetKey)
	}
...
func TestReconcileRejectsManagedSourceByOwnerReference(t *testing.T) {
	...
	assertReady(t, reconciler.Client, ctx, ref, metav1.ConditionFalse, cogniv1.ReasonManagedSourceRejected)
}
...
func TestTargetRejectedDeletesExistingManagedTarget(t *testing.T) {
	...
	assertReady(t, reconciler.Client, ctx, ref, metav1.ConditionFalse, cogniv1.ReasonTargetRejected)
}
...
func newTestReconciler(t *testing.T, objects ...client.Object) *SecretReferenceReconciler {
	...
	if err := cogniv1.AddToScheme(scheme); err != nil {
		t.Fatalf("add CogniSecrets scheme: %v", err)
	}
	return &SecretReferenceReconciler{
		Client: fake.NewClientBuilder().
			WithScheme(scheme).
			WithStatusSubresource(&cogniv1.SecretReference{}).
			WithObjects(objects...).
			Build(),
		Scheme:   scheme,
...
func reconcileOnce(t *testing.T, ctx context.Context, reconciler *SecretReferenceReconciler, ref *cogniv1.SecretReference) {
	t.Helper()
...
func newReference(namespace, name string, sources []cogniv1.SecretSource) *cogniv1.SecretReference {
	return &cogniv1.SecretReference{
		...
		Spec: cogniv1.SecretReferenceSpec{
	...
	return &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				cogniv1.AuthorizationAnnotation: allowedNamespaces,
			},
		},
...
func newManagedTarget(ref *cogniv1.SecretReference, data map[string][]byte) *corev1.Secret {
	return &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      ref.Name,
...
func assertReady(t *testing.T, c client.Client, ctx context.Context, ref *cogniv1.SecretReference, status metav1.ConditionStatus, reason string) {
	t.Helper()
...
func readyCondition(t *testing.T, c client.Client, ctx context.Context, ref *cogniv1.SecretReference) metav1.Condition {
	t.Helper()
	...
	for _, condition := range current.Status.Conditions {
		if condition.Type == cogniv1.ReadyConditionType {
			return condition
		}
	}
